...
dailyCorrelation <- function(df, df2) {
	return(list(cor(df[[2]]$close, df2[[2]]$close), cor(df[[2]]$volume, df2[[2]]$volume)))
}

weeklyCorrelation <- function(df, df2) {
	return(list(cor(df[[3]]$close, df2[[3]]$close), cor(df[[3]]$volume, df2[[3]]$volume)))
}

monthlyCorrelation <- function(df, df2) {
	return(list(cor(df[[4]]$close, df2[[4]]$close), cor(df[[4]]$volume, df2[[4]]$volume)))
}
